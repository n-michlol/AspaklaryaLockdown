...
class ALockDownAction extends FormlessAction {
	 ...
	 * @inheritDoc
	 */
	public function __construct( WikiPage $page, IContextSource $context = null ) {
	...
	protected function tempDelete() {
		...
		$this->prepareOutput( $context->msg( 'aspaklarya_lockdown-confirm', $title->getPrefixedText() ) );
		...
		if ( !$request->wasPosted() || !$user->matchEditToken( $token, [ 'aspaklarya_lockdown', $title->getPrefixedText() ] ) ) {
		...
		// TODO: from here need to write the logic
	...
	private function tempConfirmDelete(): void {
		...
		$outputPage->addWikiMsg( 'confirmaspaklaryalockdowntext' );
