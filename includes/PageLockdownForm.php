...
class PageLockdownForm {
	...
	private PageLockdownManager $main;
	...
	public function __construct( WikiPage $article, IContextSource $context ) {
		...
		if ( $this->mRequest->wasPosted() ) {
			$this->mPerformer->authorizeWrite( 'page-lockdown', $this->mTitle, $this->mPermStatus );
		} else {
			$this->mPerformer->authorizeRead( 'page-lockdown', $this->mTitle, $this->mPermStatus );
		}
		...
		$this->mReason = $this->mRequest->getText( 'pageLockdown-reason' );
		$this->mReasonSelection = $this->mRequest->getText( 'pageLockdownReasonSelection' );
		$loadBalancer = $services->getDBLoadBalancer();
		$mainObjectCache = $services->getMainWANObjectCache();
		$this->main = new PageLockdownManager( $loadBalancer, $mainObjectCache, $this->mTitle, $this->mPerformer->getUser() );
		$this->mCurrent = $this->main->getLevel();
	}

	/**
	 * PageLockdownManager entry point for action=protect and action=unprotect
	 */
	...
	private function show( $err = null ) {
		...
		if ( $this->disabled ) {
			$out->setPageTitleMsg(
				$this->mContext->msg(
					'page-lockdown-title-notallowed',
					$this->mTitle->getPrefixedText()
				)
			);
			$out->addWikiTextAsInterface(
				$out->formatPermissionStatus( $this->mPermStatus, 'page-lockdown' )
			);
		} else {
			$out->setPageTitleMsg(
				$this->mContext->msg( 'page-lockdown-title', $this->mTitle->getPrefixedText() )
			);
			$out->addWikiMsg(
				'page-lockdown-text',
	...
	private function save() {
		...
		if ( !$csrfTokenSet->matchToken( $token, [ 'page-lockdown', $this->mTitle->getPrefixedDBkey() ] ) ) {
		...
		$status = $this->main->doUpdateRestrictions(
			$this->mRequest->getText( 'mwProtect-level-pagelockdown' ),
			"$reasonstr",
		);
		...
		$this->watchlistManager->setWatch(
			$this->mRequest->getCheck( 'pageLockdownWatch' ),
			$this->mPerformer,
			$this->mTitle
		);
	...
	private function buildForm() {
		...
		$out = '';
		$fields = [];
		$levels = PageLockdownManager::getApplicableTypes( $this->mTitle->getId() > 0 );

		$section = 'restriction-pagelockdown';
		$id = 'mwProtect-level-pagelockdown';
		$options = [];
		...
		if ( !$this->disabled ) {
			...
			$fields['pageLockdownReasonSelection'] = [
				'type' => 'select',
				'cssclass' => 'pageLockdown-reason',
				'label' => $this->mContext->msg( 'pageLockdowncomment' )->text(),
				'tabindex' => 4,
				'id' => 'pageLockdownReasonSelection',
				'name' => 'pageLockdownReasonSelection',
				'flatlist' => true,
				'options' => Html::listDropDownOptions(
					$this->mContext->msg( 'pageLockdown-dropdown' )->inContentLanguage()->text(),
					[ 'other' => $this->mContext->msg( 'pageLockdown-otherreason-op' )->inContentLanguage()->text() ]
				),
				'default' => $this->mReasonSelection,
			];
			$fields['pageLockdown-reason'] = [
				'type' => 'text',
				'id' => 'pageLockdown-reason',
				'label' => $this->mContext->msg( 'pageLockdown-otherreason' )->text(),
				'name' => 'pageLockdown-reason',
				'size' => 60,
				'maxlength' => $maxlength,
			...
			if ( $this->mPerformer->getUser()->isRegistered() ) {
				$fields['pageLockdownWatch'] = [
					'type' => 'check',
					'id' => 'pageLockdownWatch',
					'label' => $this->mContext->msg( 'watchthis' )->text(),
					'name' => 'pageLockdownWatch',
					'default' => (
		...
		if ( $this->mPerformer->isAllowed( 'editinterface' ) ) {
			...
			$link = $linkRenderer->makeKnownLink(
				$this->mContext->msg( 'pageLockdown-dropdown' )->inContentLanguage()->getTitle(),
				$this->mContext->msg( 'pageLockdown-edit-reasonlist' )->text(),
				[],
				[ 'action' => 'edit' ]
			);
			$out .= '<p class="mw-pageLockdown-editreasons">' . $link . '</p>';
		}

		$htmlForm = HTMLForm::factory( 'ooui', $fields, $this->mContext );
		$htmlForm
			->setMethod( 'post' )
			->setId( 'mw-PageLockdown-Form' )
			->setTableId( 'mw-pageLockdown-table2' )
			->setAction( $this->mTitle->getLocalURL( 'action=page-lockdown' ) )
			->setSubmitID( 'mw-PageLockdown-submit' )
			->setSubmitTextMsg( 'confirm' )
			->setTokenSalt( [ 'page-lockdown', $this->mTitle->getPrefixedDBkey() ] )
			->suppressDefaultSubmit( $this->disabled )
			->setWrapperLegendMsg( 'pageLockdown-legend' )
			->prepareForm();
	...
	private function getOptionLabel( $limit ) {
		if ( $limit == '' ) {
			return $this->mContext->msg( 'pagelockdown-default' )->text();
		}
		$msg = $this->mContext->msg( "pagelockdown-level-{$limit}" );
		if ( $msg->exists() ) {
			return $msg->text();
		}
		return $this->mContext->msg( 'pagelockdown-fallback', $limit )->text();
	}

	/**
	 * Show pagelockdown long extracts for this page
	 */
	private function showLogExtract() {
		# Show relevant lines from the protection log:
		$pageLockdownLogPage = new LogPage( 'pagelockdown' );
		$this->mOut->addHTML( Xml::element( 'h2', null, $pageLockdownLogPage->getName()->text() ) );
		/** @phan-suppress-next-line PhanTypeMismatchPropertyByRef */
		LogEventsList::showLogExtract( $this->mOut, 'pagelockdown', $this->mTitle );
	}
