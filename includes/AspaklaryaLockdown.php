...
class AspaklaryaLockdown {
	...
	public static function onGetUserPermissionsErrors($title, $user, $action, &$result) {
		...
		if ($action === "edit") {
			...
			if ($pageElimination === true) {
				$result = ["aspaklarya_lockdown-error", implode(', ', self::getLinks('aspaklarya-edit-locked')), wfMessage($action)];
				return false;
			}
			if ($oldId == 0) {
				return;
		...
		if ($cachedData === 1) {
			$result = ["aspaklarya_lockdown-error", implode(', ', self::getLinks('aspaklarya-read-locked')), wfMessage($action)];
			return false;
		}
		if ($oldId > 0) {
			$locked = ALDBData::isRevisionLocked($oldId);
			if ($locked === true) {
				$result = ["aspaklarya_lockdown-rev-error", implode(', ', self::getLinks('aspaklarya-read-locked')), wfMessage($action)];
				return false;
			}
